...
- `desk_assistant/work_profiles.py` — per-exercise rule sets
  (squat depth threshold, pushup elbow angle, deadlift hip
  hinge angle).
- `desk_assistant/desk_assistant.py` — review a whole session,
  return a `CoachBriefing` with per-rep scores + session cues.
- `sim/synthetic_desk.py` — generate a reproducible session
...
- **No calibration against real athletes.** Thresholds in
  `work_profiles.py` are drawn from published squat-depth /
