...
**triage4-desk** sibling — single-session form-coaching review (per-rep
symmetry, depth, tempo). Talks to the FastAPI backend at
`triage4_desk.ui.dashboard_api` on `http://127.0.0.1:8000` by default.

This file is **copy-paste ready** for both Linux/macOS and Windows
PowerShell.

**Note:** triage4-desk cues use a `severity` literal of
...
cd triage4-desk
pip install -e ".[ui]"
uvicorn triage4_desk.ui.dashboard_api:app --reload
```
...
# {"service":"triage4-desk","version":"0.1.0","exercise":"squat",...}
```
...
```bash
cd info150/triage4-desk/web_ui
npm install
npm run dev
```
...
cd triage4-desk
pip install -e ".[ui]"
python -m uvicorn triage4_desk.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-desk\web_ui
npm install
npm run dev
```
...
The folder `triage4-desk/` itself stays on disk. To wipe it
...
Backend code: `../triage4_desk/ui/dashboard_api.py`.

---

## 11. See also

- `../README.md` — sibling overview (DeskAssistant, wellness posture)
