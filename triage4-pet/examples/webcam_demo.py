"""triage4-pet — webcam demo (Phase 10 stationary-camera support).

Owner-camera input pilot (pet at home / vet clinic). Captures frames,
measures **inter-frame motion** as a pet-activity proxy, then runs
``PetTriageEngine`` over the synthetic demo submissions and prints
the engine output alongside the camera signal.

This is **not** real animal pose / gait detection — production deploys
need a quadruped pose extractor. The frame-source layer is the same.

Owner-uploaded pet videos are not generally PHI-equivalent but may
contain owner-identifying audio / room context. Treat owner videos
as personal data: encrypt, restrict access, follow local data
protection law.
...
from triage4_pet.perception import (  # noqa: E402
...
from triage4_pet.pet_triage.triage_engine import PetTriageEngine  # noqa: E402
from triage4_pet.sim.synthetic_submission import demo_submissions  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    print("PRIVACY NOTE: owner-uploaded pet videos may contain personal data.")
    _hr()
    ...
    print(f"[run] mean motion: {mean_motion:.4f} (pet activity proxy: {activity_proxy:.2f})")
    _hr()

    submissions = demo_submissions()
    engine = PetTriageEngine()
    print(f"running PetTriageEngine over {len(submissions)} synthetic submissions")
    for obs in submissions[:3]:
        report = engine.review(obs)
        a = report.assessment
        print(f"  pet {a.pet_token}  recommendation={a.recommendation}  "
              f"overall={a.overall:.2f}")
    _hr()
    print(f"camera-observed pet activity: {activity_proxy:.2f}")
    print("(real deployment maps motion → PetObservation gait/breathing samples)")
    _hr()
