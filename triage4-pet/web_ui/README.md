...
**triage4-pet** sibling — owner-submitted veterinary triage (each
PetReport bundles assessment + vet summary + owner messages). Talks
to the FastAPI backend at `triage4_pet.ui.dashboard_api` on
`http://127.0.0.1:8000` by default.

This file is **copy-paste ready** for both Linux/macOS and Windows
PowerShell.

**Note:** triage4-pet uses a `recommendation` literal of
`can_wait / routine_visit / see_today` (not the `alert_level` used by
most other siblings).
...
# {"service":"triage4-pet","version":"0.1.0","submission_count":...}
```
...
- three cards: can wait / routine visit / see today
- left list of pets
- right detail: 4 channel bars (gait, respiratory, cardiac, pain) + owner messages + vet summary
- Re-seed button
...
cd triage4-pet
pip install -e ".[ui]"
python -m uvicorn triage4_pet.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-pet\web_ui
npm install
npm run dev
```
...
Vite proxies: `/health`, `/report`, `/submissions`, `/demo`,
`/export.html`.
...
```bash
curl http://127.0.0.1:8000/export.html > pet.html
open pet.html        # macOS
xdg-open pet.html    # Linux
```

```powershell
Invoke-WebRequest http://127.0.0.1:8000/export.html -OutFile pet.html
start pet.html
```
...
The folder `triage4-pet/` itself stays on disk. To wipe it
...
| `port 8000 already in use`                       | Stop the other uvicorn or use `--port 8001` + `TRIAGE4_PET_API_TARGET=...`.          |
...
Backend code: `../triage4_pet/ui/dashboard_api.py`.

---

## 11. See also

- `../README.md` — sibling overview (PetTriageEngine, owner+vet output)
