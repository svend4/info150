...
const COLOR = "#5a98a4"; const BG = "#0f2329"; const INSET = "#15282f";
...
export default function CameraPanel({ onAnalyzed }: { onAnalyzed: () => void }) {
  ...
  const [windowId, set_windowId] = useState("WEBCAM_DAY");
  const [v_mob, set_v_mob] = useState(0.0);
  const [v_fall, set_v_fall] = useState(false);
    ...
    const tick = () => {
      if (v.videoWidth) {
        ...
        if (ctx) {
          ...
          if (prevRef.current) {
            setLiveDeviation(Math.min(1, Math.max(0, 1 - meanLumDiff(prevRef.current, cur) * 25)));
  ...
  const stop = () => {
    ...
    setStreaming(false); setLiveDeviation(0); prevRef.current = null;
  };
  ...
  const captureAndRun = async () => {
    ...
    const deviation = motions.length ? motions.reduce((a, b) => a + b, 0) / motions.length : 0;
    setStatus(`deviation≈${deviation.toFixed(2)} — sending…`);
    try {
      await api.cameraRun({
        window_id: windowId,
        activity_deviation: deviation,
        mobility_decline: v_mob, fall_event: v_fall,
      });
      setStatus(`done. deviation=${deviation.toFixed(2)}`);
      onAnalyzed();
  ...
  return (
    <section style={{ background: BG, borderRadius: 6, padding: 16, marginBottom: 16,
      ...
      <div style={{ background: "#3d2438", color: "#ffe0c0", padding: "6px 10px", borderRadius: 4, fontSize: 11, marginBottom: 8 }}>⚠ PRIVACY NOTE: in-home cameras are PII-adjacent. Developer-test only. Production deployments must follow local privacy law (GDPR/HIPAA/state law).</div>
      <h2 style={{ marginTop: 0, fontSize: 16 }}>Camera input (in-home)</h2>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <label style={{ fontSize: 12 }}>window id&nbsp;
          <input value={windowId} onChange={(e) => set_windowId(e.target.value)}
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ minWidth: 240, display: "flex", flexDirection: "column", gap: 8 }}>
          <Stat label="Live activity deviation (low-motion)" v={liveDeviation} />
          <Slider label="Mobility decline" value={v_mob} onChange={set_v_mob} />
          <label style={{ fontSize: 12 }}>
            <input type="checkbox" checked={v_fall} onChange={(e) => set_v_fall(e.target.checked)} /> Fall event detected
          </label>
