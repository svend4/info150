...
- `triage4_crowd.venue_monitor`:
  - `crowd_safety_bands` — Helbing 2007 + Fruin 1971
    reference defaults, tunable per zone.
  - `monitoring_engine.VenueMonitorEngine.review(venue_id,
    zones)` → `VenueReport`.
- `triage4_crowd.sim`:
  - `synthetic_venue` — deterministic multi-zone fixture
