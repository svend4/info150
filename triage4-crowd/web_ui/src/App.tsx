...
export default function App() {
  ...
  if (error) return (
    <div style={{ padding: 24, color: "#ff8c8c" }}>
      ...
      <p>Backend running? <code>uvicorn triage4_crowd.ui.dashboard_api:app</code></p>
    </div>
  );
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      <header style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>triage4-crowd</h1>
        <span style={{ opacity: 0.7 }}>
          venue <code>{report.venue_id}</code> · {report.zone_count} zones · {report.alerts.length} alerts
        </span>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#181f33", borderRadius: 6, padding: 16, minHeight: 300 }}>
          {detail ? (<>
            ...
            <Bar value={detail.flow_safety} label="Flow" />
            <Bar value={detail.pressure_safety} label="Pressure" />
            <Bar value={detail.medical_safety} label="Medical-candidate density" />
      ...
      <footer style={{ marginTop: 32, fontSize: 12, opacity: 0.5 }}>
        triage4-crowd · sibling-level dashboard · MIT license
      </footer>
    </div>
  );
}
