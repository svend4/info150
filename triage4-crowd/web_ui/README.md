...
**triage4-crowd** sibling — venue / event crowd safety (density, flow,
pressure, medical-candidate density per zone). Talks to the FastAPI
backend at `triage4_crowd.ui.dashboard_api` on `http://127.0.0.1:8000`
by default.
...
# {"service":"triage4-crowd","version":"0.1.0","venue_id":"DEMO_VENUE",...}
```
...
```bash
cd info150/triage4-crowd/web_ui
npm install
npm run dev
```
...
cd triage4-crowd
pip install -e ".[ui]"
python -m uvicorn triage4_crowd.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-crowd\web_ui
npm install
npm run dev
```
...
The folder `triage4-crowd/` itself stays on disk. To wipe it
...
Backend code: `../triage4_crowd/ui/dashboard_api.py`.

---

## 11. See also

- `../README.md` — sibling overview (VenueMonitorEngine, venue-ops framing)
