"""Tests for triage4_stroll.perception frame sources.

Real-camera factory ``build_opencv_frame_source`` is intentionally
not exercised here — opencv-python is in the optional ``[camera]``
extra and CI does not always have it installed. Instead we cover
LoopbackFrameSource + SyntheticFrameSource (always available, only
need numpy) plus the import surface.
"""
...
from triage4_stroll.perception import (
    FrameSource,
...
def test_loopback_rejects_wrong_shape():
    with pytest.raises(ValueError):
        LoopbackFrameSource([np.zeros((4, 4), dtype=np.uint8)])  # 2D
...
def test_loopback_iter_protocol():
    src = LoopbackFrameSource([_frame() for _ in range(3)])
