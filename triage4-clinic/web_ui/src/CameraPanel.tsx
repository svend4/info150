...
const COLOR = "#a04060"; const BG = "#251015"; const INSET = "#2c181c";
...
export default function CameraPanel({ onAnalyzed }: { onAnalyzed: () => void }) {
  ...
  const [liveInstability, setLiveInstability] = useState(0);
  const [patientToken, set_patientToken] = useState("WEBCAM_PATIENT");
  const [v_hr, set_v_hr] = useState(0.0);
  const [v_rr, set_v_rr] = useState(0.0);
  const [v_cough, set_v_cough] = useState(0.0);
  const [v_strain, set_v_strain] = useState(0.0);
    ...
    const tick = () => {
      if (v.videoWidth) {
        ...
        if (ctx) {
          ...
          if (prevRef.current) {
            setLiveInstability(Math.min(1, meanLumDiff(prevRef.current, cur) * 25));
  ...
  const stop = () => {
    ...
    setStreaming(false); setLiveInstability(0); prevRef.current = null;
  };
  ...
  const captureAndRun = async () => {
    ...
    const instability = motions.length ? motions.reduce((a, b) => a + b, 0) / motions.length : 0;
    setStatus(`instability≈${instability.toFixed(2)} — sending…`);
    try {
      await api.cameraRun({
        patient_token: patientToken,
        postural_instability: instability,
        hr_elevation: v_hr, rr_elevation: v_rr,
        cough_frequency: v_cough, acoustic_strain: v_strain,
      });
      setStatus(`done. instability=${instability.toFixed(2)}`);
      onAnalyzed();
  ...
  return (
    <section style={{ background: BG, borderRadius: 6, padding: 16, marginBottom: 16,
      ...
      <div style={{ background: "#3d2438", color: "#ffe0c0", padding: "6px 10px", borderRadius: 4, fontSize: 11, marginBottom: 8 }}>⚠ STRONG PRIVACY NOTE: clinic camera input is PHI-equivalent. Developer-test only. Production deploys must satisfy HIPAA / GDPR / local clinical-data law.</div>
      <h2 style={{ marginTop: 0, fontSize: 16 }}>Camera input (clinical pre-triage)</h2>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <label style={{ fontSize: 12 }}>patient token&nbsp;
          <input value={patientToken} onChange={(e) => set_patientToken(e.target.value)}
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ minWidth: 240, display: "flex", flexDirection: "column", gap: 8 }}>
          <Stat label="Live postural instability (motion)" v={liveInstability} />
          <Slider label="HR elevation" value={v_hr} onChange={set_v_hr} />
          <Slider label="RR elevation" value={v_rr} onChange={set_v_rr} />
          <Slider label="Cough frequency" value={v_cough} onChange={set_v_cough} />
          <Slider label="Acoustic strain" value={v_strain} onChange={set_v_strain} />
