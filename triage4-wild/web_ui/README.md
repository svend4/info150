...
**triage4-wild** sibling — wildlife reserve health (per-observation
gait, thermal, postural, body-condition + threat-signal). Talks to the
FastAPI backend at `triage4_wild.ui.dashboard_api` on
`http://127.0.0.1:8000` by default.

This file is **copy-paste ready** for both Linux/macOS and Windows
PowerShell.

**Note:** triage4-wild depends on `biocore` (tier-1 helpers). If you
clone fresh, install biocore first — see step 1.
...
# {"service":"triage4-wild","version":"0.1.0","reserve_id":"DEMO_RESERVE",...}
```
...
```bash
cd info150/triage4-wild/web_ui
npm install
npm run dev
```

Open `http://localhost:5173`. The dashboard shows:
- header with reserve ID + observation count + alert count
- three cards: ok / watch / urgent
- left list of observations
- right detail: 5 channel bars (gait, thermal, posture, body, threat) + ranger alerts
- Re-seed button
...
cd triage4-wild
pip install -e ".[ui]"
python -m uvicorn triage4_wild.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-wild\web_ui
npm install
npm run dev
```
...
```bash
curl http://127.0.0.1:8000/export.html > wild.html
open wild.html        # macOS
xdg-open wild.html    # Linux
```

```powershell
Invoke-WebRequest http://127.0.0.1:8000/export.html -OutFile wild.html
start wild.html
```
...
The folder `triage4-wild/` itself stays on disk. To wipe it
...
| `port 8000 already in use`                       | Stop the other uvicorn or use `--port 8001` + `TRIAGE4_WILD_API_TARGET=...`.         |
...
Backend code: `../triage4_wild/ui/dashboard_api.py`.

---

## 11. See also

- `../README.md` — sibling overview (WildlifeHealthEngine, ranger framing)
