...
**triage4-drive** sibling — driver-monitoring fleet dispatcher view
(per-window PERCLOS, distraction, incapacitation). Talks to the
FastAPI backend at `triage4_drive.ui.dashboard_api` on
`http://127.0.0.1:8000` by default.

This file is **copy-paste ready** for both Linux/macOS and Windows
PowerShell.

**Note:** unlike most other siblings, drive channel scores are
**risk scores** — higher = worse (drowsiness, distraction,
incapacitation). The UI inverts the colour ramp accordingly. Alert
level literal is `ok / caution / critical`.
...
# {"service":"triage4-drive","version":"0.1.0","session_id":"DEMO_SESSION",...}
```
...
```bash
cd info150/triage4-drive/web_ui
npm install
npm run dev
```

Open `http://localhost:5173`. The dashboard shows:
- header with session ID + window count + alert count
- three cards: ok / caution / critical
- left list of windows (indexed `w00, w01, ...`)
- right detail: 3 RISK bars (PERCLOS, distraction, incapacitation) + session-level alerts
- Re-seed button
...
cd triage4-drive
pip install -e ".[ui]"
python -m uvicorn triage4_drive.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-drive\web_ui
npm install
npm run dev
```
...
```bash
curl http://127.0.0.1:8000/export.html > drive.html
open drive.html        # macOS
xdg-open drive.html    # Linux
```

```powershell
Invoke-WebRequest http://127.0.0.1:8000/export.html -OutFile drive.html
start drive.html
```

The exported HTML is annotated with a reminder that channel scores
are RISK scores.
...
| `port 8000 already in use`                       | Stop the other uvicorn or use `--port 8001` + `TRIAGE4_DRIVE_API_TARGET=...`.        |
...
Backend code: `../triage4_drive/ui/dashboard_api.py`.
