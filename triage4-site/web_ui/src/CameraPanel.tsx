...
const COLOR = "#c47a3a"; const BG = "#241808"; const INSET = "#2c1f10";
...
export default function CameraPanel({ onAnalyzed }: { onAnalyzed: () => void }) {
  ...
  const [liveFatigue, setLiveFatigue] = useState(0);
  const [workerToken, set_workerToken] = useState("WEBCAM_WORKER");
  const [v_ppe, set_v_ppe] = useState(1.0);
  const [v_lift, set_v_lift] = useState(0.0);
  const [v_heat, set_v_heat] = useState(0.0);
    ...
    const tick = () => {
      if (v.videoWidth) {
        ...
        if (ctx) {
          ...
          if (prevRef.current) {
            setLiveFatigue(Math.min(1, Math.max(0, 1 - meanLumDiff(prevRef.current, cur) * 25)));
  ...
  const stop = () => {
    ...
    setStreaming(false); setLiveFatigue(0); prevRef.current = null;
  };
  ...
  const captureAndRun = async () => {
    ...
    const fatigue = motions.length ? motions.reduce((a, b) => a + b, 0) / motions.length : 0;
    setStatus(`fatigue≈${fatigue.toFixed(2)} — sending…`);
    try {
      await api.cameraRun({
        worker_token: workerToken, fatigue,
        ppe_compliance: v_ppe, unsafe_lifting: v_lift, heat_stress: v_heat,
      });
      setStatus(`done. fatigue=${fatigue.toFixed(2)}`);
      onAnalyzed();
  ...
  return (
    <section style={{ background: BG, borderRadius: 6, padding: 16, marginBottom: 16,
      ...
      <h2 style={{ marginTop: 0, fontSize: 16 }}>Camera input (industrial site)</h2>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <label style={{ fontSize: 12 }}>worker token&nbsp;
          <input value={workerToken} onChange={(e) => set_workerToken(e.target.value)}
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ minWidth: 240, display: "flex", flexDirection: "column", gap: 8 }}>
          <Stat label="Live fatigue (low-motion proxy)" v={liveFatigue} />
          <Slider label="PPE compliance" value={v_ppe} onChange={set_v_ppe} />
          <Slider label="Unsafe lifting" value={v_lift} onChange={set_v_lift} />
          <Slider label="Heat stress" value={v_heat} onChange={set_v_heat} />
