# triage4-sport web UI

Single-page React + TypeScript + Vite dashboard for the
**triage4-sport** sibling — elite athlete performance review (per-
session form asymmetry, workload, recovery HR, baseline deviation).
Three-audience output: coach messages + trainer notes + optional
physician alert. Talks to the FastAPI backend at
`triage4_sport.ui.dashboard_api` on `http://127.0.0.1:8000` by
default.

This file is **copy-paste ready** for both Linux/macOS and Windows
PowerShell.

**Note:** triage4-sport uses a `risk_band` literal of
`steady / monitor / hold`.
...
cd triage4-sport
pip install -e ".[ui]"
uvicorn triage4_sport.ui.dashboard_api:app --reload
```
...
# {"service":"triage4-sport","version":"0.1.0","session_count":...}
```
...
```bash
cd info150/triage4-sport/web_ui
npm install
npm run dev
```

Open `http://localhost:5173`. The dashboard shows:
- header with athlete-session count
- three cards: steady / monitor / hold
- left list of athletes (with physician-alert flag if present)
- right detail: 4 channel bars + coach messages + trainer notes + physician alert
- Re-seed button
...
cd triage4-sport
pip install -e ".[ui]"
python -m uvicorn triage4_sport.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-sport\web_ui
npm install
npm run dev
```
...
Vite proxies: `/health`, `/report`, `/sessions`, `/demo`,
`/export.html`.
...
```bash
curl http://127.0.0.1:8000/export.html > sport.html
open sport.html        # macOS
xdg-open sport.html    # Linux
```

```powershell
Invoke-WebRequest http://127.0.0.1:8000/export.html -OutFile sport.html
start sport.html
```
...
The folder `triage4-sport/` itself stays on disk. To wipe it
...
| `port 8000 already in use`                       | Stop the other uvicorn or use `--port 8001` + `TRIAGE4_SPORT_API_TARGET=...`.        |
...
Backend code: `../triage4_sport/ui/dashboard_api.py`.

---

## 11. See also

- `../README.md` — sibling overview (SportPerformanceEngine, three-audience output)
