...
const COLOR = "#a08840"; const BG = "#251f10"; const INSET = "#2a2515";
...
export default function CameraPanel({ onAnalyzed }: { onAnalyzed: () => void }) {
  ...
  const [species, set_species] = useState<string>("dairy_cow");
  const [animalId, set_animalId] = useState("WEBCAM_ANIMAL");
  const [v_lameness, set_v_lameness] = useState(0.0);
  const [v_resp, set_v_resp] = useState(0.0);
  ...
  const captureAndRun = async () => {
    ...
    try {
      await api.cameraRun({
        animal_id: animalId, species,
        activity_proxy: activity,
        lameness_severity: v_lameness, respiratory_elevation: v_resp,
  ...
  return (
    <section style={{ background: BG, borderRadius: 6, padding: 16, marginBottom: 16,
      ...
      <h2 style={{ marginTop: 0, fontSize: 16 }}>Camera input (livestock)</h2>
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select value={species} onChange={(e) => set_species(e.target.value)}
          style={{ padding: 6, background: INSET, color: "#dde7df",
            border: `1px solid ${COLOR}`, borderRadius: 4 }}>
            <option key="dairy_cow" value="dairy_cow">dairy_cow</option>
            <option key="pig" value="pig">pig</option>
            <option key="chicken" value="chicken">chicken</option>
        </select>
        <label style={{ fontSize: 12 }}>animal id&nbsp;
          <input value={animalId} onChange={(e) => set_animalId(e.target.value)}
      ...
      <div style={{ display: "flex", gap: 16, marginTop: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ minWidth: 240, display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <Slider label="Lameness severity" value={v_lameness} onChange={set_v_lameness} />
          <Slider label="Respiratory elevation" value={v_resp} onChange={set_v_resp} />
