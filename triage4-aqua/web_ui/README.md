...
**triage4-aqua** sibling — pool / beach drowning-detection support
(submersion + IDR + absent + distress channels). Talks to the FastAPI
backend at `triage4_aqua.ui.dashboard_api` on `http://127.0.0.1:8000`
by default.
...
# {"service":"triage4-aqua","version":"0.1.0","pool_id":"DEMO_POOL",...}
```
...
```bash
cd info150/triage4-aqua/web_ui
npm install
npm run dev
```

Open `http://localhost:5173`. The dashboard shows:
- header with pool ID + swimmer count
- three cards: ok / watch / urgent
- left list of swimmers
- right detail: 4 channel bars (submersion, IDR, absent, distress) + alerts
- Re-seed button
...
cd triage4-aqua
pip install -e ".[ui]"
python -m uvicorn triage4_aqua.ui.dashboard_api:app --reload
```
...
```powershell
cd C:\Users\<your-username>\info150\triage4-aqua\web_ui
npm install
npm run dev
```
...
Vite proxies: `/health`, `/report`, `/swimmers`, `/alerts`, `/demo`,
`/export.html`.
...
```bash
curl http://127.0.0.1:8000/export.html > aqua.html
open aqua.html        # macOS
xdg-open aqua.html    # Linux
```

```powershell
Invoke-WebRequest http://127.0.0.1:8000/export.html -OutFile aqua.html
start aqua.html
```
...
The folder `triage4-aqua/` itself stays on disk. To wipe it
...
| `port 8000 already in use`                       | Stop the other uvicorn or use `--port 8001` + `TRIAGE4_AQUA_API_TARGET=...`.         |
...
Backend code: `../triage4_aqua/ui/dashboard_api.py`.

---

## 11. See also

- `../README.md` — sibling overview (PoolWatchEngine, lifeguard framing)
