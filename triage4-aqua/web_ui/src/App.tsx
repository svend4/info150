...
export default function App() {
  ...
  const load = async () => {
    try { setError(null); const d = await api.report(); setReport(d);
      if (!selected && d.scores.length) setSelected(d.scores[0].swimmer_token); }
    catch (e) { setError((e as Error).message); }
  };
  useEffect(() => { load(); }, []);
  useEffect(() => {
    if (!selected) return;
    api.swimmer(selected).then(setDetail).catch((e) => setError((e as Error).message));
  }, [selected]);

  const reload = async () => {
    await api.reload(); setSelected(null); setDetail(null); await load();
  };

  if (error) return (
    <div style={{ padding: 24, color: "#ff8c8c" }}>
      Error: <code>{error}</code>
      <p>Backend running? <code>uvicorn triage4_aqua.ui.dashboard_api:app</code></p>
    </div>
  );
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      <header style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 16 }}>
        <h1 style={{ margin: 0, fontSize: 22 }}>triage4-aqua</h1>
        <span style={{ opacity: 0.7 }}>
          pool <code>{report.pool_id}</code> · {report.swimmer_count} swimmers · {report.alerts.length} alerts
        </span>
      ...
      <section style={{ display: "flex", gap: 12, marginBottom: 24, flexWrap: "wrap" }}>
        {(["ok", "watch", "urgent"] as const).map((lvl) => (
          <div key={lvl} style={{ flex: 1, minWidth: 160, background: "#162132",
      ...
      <div style={{ display: "grid", gridTemplateColumns: "320px 1fr", gap: 16 }}>
        <div style={{ background: "#162132", borderRadius: 6, padding: 8, maxHeight: 600, overflowY: "auto" }}>
          {report.scores.map((s) => (
            <button key={s.swimmer_token} onClick={() => setSelected(s.swimmer_token)}
              style={{ display: "block", width: "100%", textAlign: "left", padding: "8px 12px",
                margin: "2px 0", background: selected === s.swimmer_token ? "#243250" : "transparent",
                color: "#e0eaf2", border: 0, borderLeft: `4px solid ${LEVEL_COLOR[s.alert_level]}`,
                borderRadius: 4, cursor: "pointer", fontSize: 13 }}>
              <div style={{ fontWeight: 600 }}>{s.swimmer_token}</div>
        ...
        <div style={{ background: "#162132", borderRadius: 6, padding: 16, minHeight: 300 }}>
          {detail ? (<>
            <h2 style={{ marginTop: 0, fontSize: 18 }}>
              {detail.swimmer_token}{" "}
            ...
            <Bar value={detail.submersion_safety} label="Submersion safety" />
            <Bar value={detail.idr_safety} label="Instantaneous distress (IDR)" />
            <Bar value={detail.absent_safety} label="Presence" />
            <Bar value={detail.distress_safety} label="Distress signature" />
          ...
          </>) : <p style={{ opacity: 0.6 }}>Select a swimmer to see channels + alerts.</p>}
        </div>
      </div>
      <footer style={{ marginTop: 32, fontSize: 12, opacity: 0.5 }}>
        triage4-aqua · sibling-level dashboard · MIT license
      </footer>
    </div>
  );
}
