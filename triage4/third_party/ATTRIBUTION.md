...
| `svend4/meta2` · `puzzle_reconstruction/matching/orient_matcher.py` | `triage4/matching/orient_matcher.py` | Copied verbatim: `OrientConfig`, `OrientProfile`, `OrientMatchResult`, `compute_orient_profile` (Sobel-gradient orientation histogram), `orient_similarity`, `best_orient_angle`, `match_orient_pair`, `batch_orient_match`. Used in triage4 to align casualty silhouettes / wound regions to a common axis (gravity, body axis, camera horizon) before downstream signature extraction. |
| `svend4/meta2` · `puzzle_reconstruction/scoring/match_evaluator.py` | `triage4/scoring/match_evaluator.py` | Copied verbatim: `EvalConfig`, `MatchEval` (with precision / recall / f1), `EvalReport`, `compute_precision`, `compute_recall`, `compute_f_score`, `evaluate_match`, `evaluate_batch_matches`, `aggregate_eval`, `filter_by_score` (re-exported as `filter_eval_by_score`), `rank_matches`. Enables precision / recall / F-β evaluation of triage decisions against ground-truth labels — required for DARPA-style benchmarking. |
| `svend4/meta2` · `puzzle_reconstruction/scoring/global_ranker.py` | `triage4/scoring/global_ranker.py` | Copied verbatim: `RankedPair` (re-exported as `GlobalRankedPair`), `RankingConfig`, `normalize_matrix`, `aggregate_score_matrices`, `rank_pairs` (re-exported as `global_rank_pairs`), `top_k_candidates`, `global_rank`, `score_vector`, `batch_global_rank`. Lets triage4 merge multiple signature-similarity matrices (motion, perfusion, bleeding, thermal, …) into one consensus ranking of casualty pairs. |
